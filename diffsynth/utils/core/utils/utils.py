...
def coords_grid(batch, ht, wd, device, dtype=torch.float32):
...
def check_cycle_consistency(flow_01, flow_10, alpha=0.01, beta=0.5):
    H, W = flow_01.shape[:2]
    coords = coords_grid(1, H, W, flow_01.device, dtype=flow_01.dtype)
    coords = coords.permute(0, 2, 3, 1)[0]  # [H, W, 2]
    coords_1 = coords + flow_01
    coords_1_int = torch.round(coords_1).long()
    flow_reprojected = bilinear_sampler(flow_10, coords_1_int.permute(0, 2, 3, 1))
    cycle = flow_reprojected + flow_01
    cycle = torch.norm(cycle, dim=1)
    mask = (cycle < alpha * min(H, W)).float()
    return mask[0].numpy()